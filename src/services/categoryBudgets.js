...
export async function saveCategoryBudget(categoryId, amount, month, year) {
  ...
  if (existing.rows.length > 0) {
    ...
    await executeSql(
      `UPDATE category_budgets SET amount = ?, updated_at = datetime('now') WHERE id = ?`,
      [amount, id]
    );
    return id;
  } else {
    // Insert
    const res = await executeSql(
      `INSERT INTO category_budgets (category_id, amount, month, year, created_at, updated_at) VALUES (?,?,?,?,datetime('now'),datetime('now'))`,
